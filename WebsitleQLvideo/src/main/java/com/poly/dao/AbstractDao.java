...
public class AbstractDao<T> {
	...
	public List<T> findAll(Class<T> clazz, boolean existIsActive, int pageNumber, int pageSize) { // phân trang
		String entityName = clazz.getSimpleName();
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT o FROM ").append(entityName).append(" o");
		if (existIsActive == true) {
		...
		query.setMaxResults(pageSize); // // vị trí kết thúc
		return query.getResultList();
	}

	
	// select * from user o where o.username = ?0 and o.passowrd = ? 1;
	// findOne(User.class ,sql , "HungSmall","hungnho")
